  ...
  return (
    <div
      ...
      className={`flex flex-col items-center min-w-0 ${
        ...
        first ? 'w-[6.5rem] sm:w-32 mb-6' : 'w-[5.5rem] sm:w-28'
      }`}
    >
      <div className={`relative ${first ? 'podium-float' : ''}`}>
        ...
        <div className={`podium-ring ${RING_CLASS[medal]}`} style={{ width: size, height: size }}>
          <span className="podium-ring-metal" aria-hidden="true" />
          <span className={`podium-ring-face ${first ? 'text-[26px]' : 'text-[21px]'}`} aria-hidden="true">
      ...
      <div
        className="mt-4 w-full truncate text-center text-xs sm:text-sm font-bold text-[var(--color-text)]"
      ...
      <div className="mt-1 flex items-center gap-1 text-[var(--color-cta)]">
        ...
        <span className="font-mono text-sm font-bold">{entry.points}</span>
      </div>
      {entry.isMe && (
...
export default function Podium({
  ...
  return (
    <div
      ...
      className="card flex items-end justify-center gap-2 sm:gap-5 px-4 pt-9 pb-6 sm:px-6"
    >
