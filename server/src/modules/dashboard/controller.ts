...
import { OffersQuerySchema, TimeseriesQuerySchema } from './schemas.js'
...
export async function offers(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!req.user?.merchantId) throw unauthenticated('未登录')
    const merchantId = req.user.merchantId
    const result = await dashboardService.getTopOffers(merchantId, parsed.data.range)
    res.json(result)
  } catch (err) {
    next(err)
  }
