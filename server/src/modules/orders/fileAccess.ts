 ...
 *
 * 语义要点（设计 §4/§5）：
 * - 防枚举：订单不存在 / 不属于请求者 → 统一 404「订单不存在」，不可区分。
 *   归属已确认但无文件交付 → 同为 404/NOT_FOUND，文案「该订单没有可下载的
 *   交付文件」（只有本就能看到该订单的人才会读到，不构成探针）。只有确认
 *   归属后被状态规则拒绝才 403。
...
async function resolveGrant(
  ...
  // 归属已确认（买家本人 / 本单商家 / 管理员）但本单没有文件交付：仍是 404
  // + NOT_FOUND（不落审计、不改状态码），只把文案说准。陌生人在上面那条
  // 分支已经拿到「订单不存在」，因此这句话不会成为新的存在性探针。
  if (!file) {
    return { error: notFound('该订单没有可下载的交付文件') }
  }
