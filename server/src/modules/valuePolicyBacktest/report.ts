...
export function buildReport(input: ValidatedInput, options: RunOptions): BacktestReport {
  ...
  const report: BacktestReport = {
    ...
    metadata: {
      ...
      gitCommit: options.runtime.gitCommit(),
...
export function renderMarkdown(report: BacktestReport): string {
  const lines: string[] = [
    ...
    '## Sensitivity matrix',
    '',
    '| PTS / CNY | Offer P50 CNY | Offer P90 CNY | Monthly reward CNY | Balance reference-value exposure CNY | Rounding incidence | Below-cost rate | P50 offers / median earned | vs 100 PTS/CNY |',
    '| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |',
    ...report.sensitivity.map(row => `| ${row.pointsPerCnyMajor} | ${cell(row.offerP50ReferenceCny)} | ${cell(row.offerP90ReferenceCny)} | ${cell(row.monthlyRewardReferenceCny)} | ${cell(row.totalBalanceReferenceValueExposureCny)} | ${cell(row.roundingIncidence)} | ${cell(row.belowCostOfferRate)} | ${cell(row.p50OffersBuyableWithMedianEarned)} | ${cell(row.multiplierVs100PtsPerCny)} |`),
    '',
  ]
  ...
  for (const candidate of report.candidates) {
    ...
    lines.push(`- reference-value exposure CNY: ${candidate.balances.referenceValueExposureCny}`)
  ...
  lines.push('')
  lines.push('D-02 STATUS: NOT APPROVED')
  lines.push('')
  return `${lines.join('\n')}`
}
