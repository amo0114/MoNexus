 ...
 *
 * 硬规则：
 * - 候选：bookingDate 非空、status ∈ {pending, processing}、预约日在
 *   (now, now + 24h] 内；商家归属非空（平台自营无商家收件人）。
  ...
  try {
    const orders = await prisma.order.findMany({
      where: {
        ...
        // 预约日在未来 24 小时内（不含已过期的预约——过期提醒无意义）。
        bookingDate: { gt: now, lte: new Date(now.getTime() + DAY_MS) },
