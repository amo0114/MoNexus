...
Review date: 2026-09-02. Scope: M5 production and staging secret inventory for frontend, backend, PostgreSQL, JWT, MFA, registration-abuse protection, Redis, same-origin ALTCHA (default) with optional Turnstile adapter, Sentry, SMTP, S3-compatible storage, metrics, backup, deploy, and alert routing. No secret values belong in this repository.
...
| `HUMAN_VERIFICATION_PROVIDER` | Environment variable | `staging`, `production` | Security/Ops | Production must be `altcha` (default) or `turnstile`. `off` is not allowed in production/enforce | Environment variables / host env | backend runtime; `scripts/check-prod-env.sh` preflight |
| `ALTCHA_HMAC_KEY` | Secret | `staging`, `production` | Security/Ops (primary), Backend lead (backup) | Independent canonical standard-base64 encoding of exactly 32 random bytes used to sign same-origin ALTCHA challenges. Must not reuse `JWT_SECRET`, `MFA_ENCRYPTION_KEY`, or `ABUSE_HASH_KEY`. Required when `HUMAN_VERIFICATION_PROVIDER=altcha` | Environment secrets; every API instance in one environment receives the same value | backend runtime; `scripts/check-prod-env.sh` preflight |
| `TURNSTILE_SITE_KEY` | Environment variable | `staging`, `production` (optional adapter) | Security/Ops | Public Cloudflare Turnstile site key; required only when `HUMAN_VERIFICATION_PROVIDER=turnstile`. May appear only in the safe registration-status challenge DTO | Environment variables / host env | backend runtime, public login page response |
| `TURNSTILE_SECRET_KEY` | Secret | `staging`, `production` (optional adapter) | Security/Ops | Server-only Cloudflare siteverify credential; required only when the Turnstile adapter is selected. Never sent to browser, logs, audit, metrics, or tickets | Environment secrets | backend runtime; `scripts/check-prod-env.sh` preflight |
| `TURNSTILE_ALLOWED_HOSTNAMES` | Environment variable | `staging`, `production` (optional adapter) | Security/Ops | Comma-separated exact public hostnames; required only when the Turnstile adapter is selected. No scheme, path, port, wildcard, or URL | Environment variables / host env | backend runtime; `scripts/check-prod-env.sh` preflight |
...
| `STORAGE_UI_CONFIG_ENABLED` | Environment variable | `staging`, `production` | Storage/Ops | When `false`, admin storage console is read-only | Environment variables | backend runtime (SPEC-STORAGE-001) |
| `STORAGE_CONFIG_SOURCE` | Environment variable | `staging`, `production` | Storage/Ops | `database` allows active provider overlay; `env` forces bootstrap only (circuit breaker) | Environment variables | backend runtime (SPEC-STORAGE-001) |
| `STORAGE_CREDENTIALS_ENC_KEY` | Secret | `staging`, `production` | Storage/Ops (primary), Security backup | 64 hex chars (32 bytes) AES-256-GCM key for cloud storage AK/SK at rest; never reuse webhook key material | Environment secrets | backend runtime (SPEC-STORAGE-001) |
| `STORAGE_CREDENTIALS_ENC_KEY_VERSION` | Environment variable | `staging`, `production` | Storage/Ops | Positive integer key version embedded in ciphertext prefix | Environment variables | backend runtime |
| `STORAGE_CREDENTIALS_ENC_KEY_PREVIOUS` | Secret | `staging`, `production` | Storage/Ops | Optional previous 32-byte hex key during rotation; decrypts old `v{n}` ciphertext | Environment secrets | backend runtime (SPEC-STORAGE-001) |
| `STORAGE_CREDENTIALS_ENC_KEY_PREVIOUS_VERSION` | Environment variable | `staging`, `production` | Storage/Ops | Version `n` that `PREVIOUS` decrypts; required when PREVIOUS is set | Environment variables | backend runtime |
| `STORAGE_ENDPOINT_ALLOWLIST` | Environment variable | `staging`, `production` | Storage/Ops | Comma-separated hosts: single-label = exact match only (e.g. `minio`); multi-label = exact or subdomain suffix (e.g. `minio.internal`). Never bare TLDs (`com`) | Environment variables | backend runtime |
...
9. Generate `ALTCHA_HMAC_KEY` in a third approved session with the same canonical 32-byte base64 requirement. It must be independent of `JWT_SECRET`, `MFA_ENCRYPTION_KEY`, and `ABUSE_HASH_KEY`. Set `HUMAN_VERIFICATION_PROVIDER=altcha` for the default production path. Turnstile is optional: keep its secret/site/hostname trio only while rolling from Turnstile or as a rollback adapter (`HUMAN_VERIFICATION_PROVIDER=turnstile`). Never treat Turnstile as the only production path.
...
| `ALTCHA_HMAC_KEY` | Suspected HMAC exposure or scheduled rotation | Generate a new independent 32-byte key, deploy it to every API instance together, and treat in-flight challenges as invalid. Do not reuse JWT, MFA, or `ABUSE_HASH_KEY` material. |
| Turnstile secret (optional adapter) | Provider compromise, hostname change, scheduled rotation, or rollback to the Turnstile adapter | Create/rotate the Turnstile secret in the provider console, update the environment secret and exact hostname allow-list, deploy, then validate on the real staging hostname. Never place the secret in frontend build variables. Not required when `HUMAN_VERIFICATION_PROVIDER=altcha`. |
| Redis credential | Redis access leak, operator departure, or managed-service rotation | Rotate the self-hosted Redis password or provider credential, update `REDIS_URL`/`REDIS_PASSWORD` atomically on every backend instance, deploy, and verify registration fails closed during any unavailable window rather than falling back locally. |
...
- Security/Ops: own the separate `ABUSE_HASH_KEY` and `ALTCHA_HMAC_KEY`. Keep optional Turnstile credentials only while that adapter is selected or retained for rollback. Never add a protection-bypass setting to a production environment; use the registration switch and incident process when dependencies are unhealthy.
