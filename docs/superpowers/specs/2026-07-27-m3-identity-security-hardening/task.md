...
| T-BE-03 | 实现管理员 MFA 登录、绑定与恢复码 API | P0 | L | T-BE-02 | B | F-010–016 |
| T-BE-04 | 实现稳定会话族与会话管理 API | P0 | L | T-BE-01 | C | F-020–024 |
...
| T-FE-02 | 实现账户安全与设备会话 UX | P0 | M | T-BE-03, T-BE-04, P6a→develop 后 rebase | D | F-015, F-021–024 |
...
- [ ] 向 User、RefreshToken 添加 spec §5.1 所需字段、索引和关系。
- [ ] 新增 MfaRecoveryCode、AuthChallenge、SecurityEvent 模型，确认 FK 删除语义不会意外移除审计。
- [ ] 为 MFA_ENCRYPTION_KEY 建立严格 base64 32-byte env parser；production 缺失/格式错误启动失败，test 有隔离配置。
- [ ] 执行 prisma migrate dev --name identity_security_hardening，不手改生成 migration。
- [ ] 在新迁移的隔离库检查所有历史 RefreshToken 有唯一非空 sessionId；若做两步 nullable migration，写明第二步收紧任务及临时兼容规则。
- [ ] 增加 migration/config guard 测试。

**DoD：** schema generate、migration、migrate status、config production guard 通过；无 secret default。
...
### T-BE-03 — 实现管理员 MFA 登录、绑定与恢复码 API
...
- [ ] 添加 enrollment start / confirm、MFA verify、recovery code regenerate、reconfigure start / confirm schema 和 route。
...
- [ ] 重新绑定必须要求当前密码和已有 MFA 因子；成功 bump mfaVersion 并吊销其他会话。
...
- [ ] revoke-others 排除 current sessionId；revoke-all 吊销全部、响应后 controller 清当前 cookie。
- [ ] 所有吊销写 session_revoked SecurityEvent，reason 采用受控枚举。
- [ ] 保持 refresh replay 的 revoke-all-user 语义，并记录 session_replay_detected。
- [ ] 写两 session、两用户、轮换继承、单吊销、其他/全部吊销、replay 回归测试。
...
- [ ] 当前设备不可误显示为“其他”；单吊销/其他/全部均有确认与 loading 防重。
- [ ] 删除当前/全部成功后调用 logout 并导航；删除其他成功后 re-fetch。
- [ ] 管理员显示 MFA 已开启、恢复码剩余数、重新生成恢复码、换机入口；不显示 seed。
...
- [ ] migration invariant：legacy RefreshToken 都有可用 sessionId，新增索引/约束可用。
- [ ] 运行完整 server test。
...
- [ ] 跑 npm run verify:local:no-e2e、npm run e2e、npm --prefix server run build、npm run build。
- [ ] 在隔离库跑 npx prisma migrate status；记录命令和结果。
...
- [ ] runbook 添加 key 配置、管理员首次绑定、双人 break-glass 与事件审查；不写真实 secret、recovery code 或 token。
