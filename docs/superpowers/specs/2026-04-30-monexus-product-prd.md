...
| 4.3.1 | 商家履约 `processing → delivered`（仅 `manual_service`） | ✅ | `deliverOrderFulfillment` |
...
| § | 缺项 | 实施提示 |
| --- | --- | --- |
| 4.4.1 | `Product.isRecurring` / `billingCycleDays` / `autoRenew`、`Subscription` 表、续费 cron、`past_due` 宽限、暂停/恢复/取消 | 订阅 cron 必须复用积分冻结模型，避免续费失败时积分已扣 |
| 4.4.2 | 满减券、限时秒杀、双倍积分日 | 仅以站内积分表达，禁止与法币挂钩 |
| 4.4.3 | 邀请人数排行榜、奖励规则可配置 | 复用 `SystemConfig`，避免再写死 |
| 4.4.4 | 数据看板：DAU/WAU/MAU、GMV、客单价、复购率、商家排行、SLA 达成率 | 基于 `OrderStatusEvent` + `PointLog` 聚合，不另起数据仓库 |
