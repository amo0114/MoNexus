...
| 2026-08-10 00:39 CST | `d7e753b2b23d6f57c8d021e6c28fc881aa5e35c6` | I-RT-005/010；I-RT-011 local CI evidence | CHK-SSE-001/008、CHK-QA-015~016；G-PR-004~005 | `API_RATE_LIMIT_MAX=3000` 的默认 CI-equivalent Playwright；`bash scripts/verify-notification-realtime-e2e.sh`；targeted realtime stream；完整 `verify-notification-realtime.sh --local` | 默认 Playwright 110 passed / 6 skipped，realtime 两个 spec 仅由专用 config 收集；专用 realtime Playwright 10/10；stream 12/12 并断言未鉴权连接暴露 dedicated `30;w=60` policy、不会消耗 REST 15 分钟预算；完整 verifier exit 0（backend 152、frontend 60、Playwright 10/10、multi-instance、drift/secret/clean boundary 全绿） | commit `d7e753b`；`.github/workflows/ci.yml` 新增独立 `Notification realtime Playwright E2E` job（专用 DB、guarded script）并纳入 `CI OK`；默认 `playwright.config.ts` ignore 两个 realtime spec。外部 release gates 仍 Pending |
| 2026-08-10 01:39 CST | `d4d4fd0436f3f4717b51318f3826dfa6eb3f046a` | I-RT-010；I-RT-011 PR CI evidence | CHK-QA-015~016；G-PR-003~005/010 | GitHub Actions PR run `31326131305`（全部 jobs + aggregate）；检查前序诊断 runs `31324811368` / `31324893415` | **overall success / CI OK success**；backend 127 files / 1096 tests（627.31s，job 11m11s）；frontend build success；legal E2E 10/10（20.1s）；默认 Playwright 110 passed / 6 skipped（4.8m）；专用 realtime Playwright 10/10（1.2m，专用 DB `monexus_test_notification_realtime`）；Compose deployment contract success。前序 workflow-context failure、backend 15m cancellation 与单-IP late 429 均有失败证据并分别由 step-runtime env、20m bounded cap、IPv4 browser / IPv6 API-fixture 双真实 loopback client 修复；`API_RATE_LIMIT_MAX` 保持 3000，无 limiter bypass | [run 31326131305](https://github.com/amo0114/MoNexus/actions/runs/31326131305)；`Notification realtime Playwright E2E` 与最终 `CI OK` 均 success。AC-RT-025/029、deployed proxy、rollout/rollback、Owner gate 仍 Pending |

证据规则：

- 命令必须含 exit code、测试数与耗时；
- 环境证据含 DB 名、ports、feature flags，不含密码 / token；
- E2E artifact 策略：trace、screenshot、video 全部关闭，避免 Bearer、登录密码或 delivery secret 泄漏；证据只使用脱敏 list reporter/stdout，禁止声称有 trace。
...
| G-PR-003 | backend / frontend build 全绿 | Passed | code/workflow HEAD `d4d4fd0`：GitHub run `31326131305` backend 127 files / 1096 tests + frontend build success；本地 final verify builds 亦通过 |
| G-PR-004 | 既有通知、订单、auth、announcement 回归全绿 | Passed | GitHub run `31326131305`：backend 1096/1096、legal E2E 10/10、默认 Playwright 110 passed / 6 skipped；`API_RATE_LIMIT_MAX=3000` 未提高，浏览器与 API fixture 使用两个真实 loopback client bucket |
| G-PR-005 | 新 realtime client / E2E / multi-instance / proxy suite 全绿 | Passed | GitHub run `31326131305`：独立 `Notification realtime Playwright E2E` 10/10 + aggregate `CI OK` success；本地 multi-instance、session/proxy self-tests PASS。实际部署 proxy/log smoke 仍由 G-PR-006/009 阻断 release |
...
| G-PR-010 | PR 描述含规格链接、配置、监控、风险、回滚与证据索引 | Passed | PR #130 描述覆盖规格链接、默认关闭、配置、监控、风险、回滚、run `31326131305` 与 Pending release-only gates；证据索引见 Evidence Ledger 与 checklist |
