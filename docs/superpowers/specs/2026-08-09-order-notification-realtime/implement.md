...
| G-PR-004 | 既有通知、订单、auth、announcement 回归全绿 | Passed | code HEAD `d7e753b`：backend 16 files / 152 tests、frontend 8 files / 60 tests；默认 CI-equivalent Playwright 在 `API_RATE_LIMIT_MAX=3000` 下 110 passed / 6 skipped |
| G-PR-005 | 新 realtime client / E2E / multi-instance / proxy suite 全绿 | Passed | code HEAD `d7e753b`：专用 Playwright 10/10、multi-instance PASS、session/proxy self-tests PASS；workflow 已把专用 DB/ports/guarded-script realtime job 纳入 `CI OK`。实际部署 proxy/log smoke 仍由 G-PR-006/009 阻断 release |
