...
| 2026-08-15 | `b078153` (docs-only; code ancestor `c690025`) | C5b | `CHK-MERCH-FND-002` / `G-MERCH-PR-003` | Production-source `isHot` scan; attempted Node 20 disposable-DB legacy-hot tests | Static scan PASS: no production `isHot: true` data writes; public/admin projection strips legacy column. DB run invalidated by PostgreSQL `40P01` setup deadlock and wrong-runtime retry; no true-count→false-count cleanup artifact, so both remain Pending | `docs/specs/cmi-c5b-evidence.md`; CMI dbguard cleanup |
