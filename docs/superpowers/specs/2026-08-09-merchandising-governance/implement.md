...
| 2026-08-16 | `4ab3de9` / `e329d1b` / `440337b` | C5b | `G-MERCH-PR-009` | `bash scripts/verify-cmi-perf-compat.sh`; `bash scripts/verify-cmi-100k-order-p95.sh`; `npm run check:bundle-budget`; protected staging rehearsal `31929179740` | Local cache/build/100k benchmark PASS; staging canary P95 PASS (`p95_ms=794`, zero failures), fallback and rollback PASS. The 315.74 KiB all-assets proxy is explicitly Deferred by AMD-CMI-012 §3.6 and is not represented as shipped budget functionality | `docs/specs/cmi-perf-compat-evidence.md`; workflow artifact `notification-realtime-staging-evidence-440337b...` |
| 2026-08-16 | `440337b` / `31929179740` | C5b | `G-MERCH-PR-010` | Protected staging release rehearsal and Owner/PAR handoff; `bash -n` local checks | PASS for staging scope: Owner approval, exact-SHA rollout, canary, readiness, rollback, fixture cleanup, and evidence retention all recorded; production promotion/restore remains out of scope | `docs/specs/cmi-release-rehearsal.md`; `docs/specs/cmi-owner-handoff.md` |
...
| G-MERCH-PR-006 | browser/disclosure/a11y/visual/assets通过 | Passed (visual/assets Deferred) |
| G-MERCH-PR-007 | AC-MERCH-001~029当前HEAD证据齐全 | Pending |
| G-MERCH-PR-008 | points/PII/secret/禁词/public field审计通过 | Passed |
| G-MERCH-PR-009 | 性能/bundle/cache/rollout/rollback通过 | Passed (asset/bundle scope Deferred) |
| G-MERCH-PR-010 | points/orders/products/admin/notification回归与PR交接完整 | Passed (staging scope) |
