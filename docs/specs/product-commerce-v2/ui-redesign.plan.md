...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Light** (科技白) | `var(--color-background)` (`#F8FAFC`) | `var(--color-surface)` (`#FFFFFF`) | `var(--color-primary)` (`#4F46E5`) | `var(--color-cta)` (`#15803D` 底 / `#FFFFFF` 字, 5.02:1 AA) | `#0F172A` / `#64748B` | 标准圆角 (8–16px)，利落现代 |
| **Dark** (夜空蓝黑) | `var(--color-background)` (`#0A0A14`) | `var(--color-surface)` (`#161629`) | `var(--color-primary)` (`#818CF8`) | `var(--color-cta)` (`#4ADE80` 底 / `#0A0A14` 字, 11.30:1 AAA) | `#F1F5F9` / `#94A3B8` | 高对比赛博深邃，光晕阴影 |
| **Soft** (暖阳治愈) | `var(--color-background)` (`#FFF8EC`) | `var(--color-surface)` (`#FFFEFB`) | `var(--color-primary)` (`#FF8C42`) | `var(--color-cta)` (`#AD4B35` 暖陶红底 / `#FFFFFF` 字, 5.48:1 AA) | `#5D3A1F` / `#A07050` | 大圆角 (`--radius-xl: 22px`)，柔和暖意 |
| **Ink** (墨韵默认) | `var(--color-background)` (`#EEF0EE`) | `var(--color-surface)` (`#F8F9F7`) | `var(--color-primary)` (`#34507A`) | `var(--color-cta)` (`#3D7257` 松绿底 / `#FFFFFF` 字, 5.60:1 AA) | `#22262C` / `#666E77` | 宣纸冷白、松烟墨、绫绢灰边 |
...
* **双栏布局**：
  * 左侧内容区：`flex-1 min-w-0`，容纳 4:3 原图 contain 画廊。**画廊采用纯净无字几何封面素材，页码指示器（如 1/2）与灯箱放大按钮全部由前端 React/HTML 组件独立渲染，严禁使用带界面文字与按钮的截图作为底图**；缩略图导轨展示独立商品图片；富文本图文介绍、规格属性表、使用说明、FAQ 顺次排布；
...
* **信息优先与首屏展示**：
  ...
  * 紧随其后为 4:3 比例的克制画廊（包含 1/2 指示与放大图标，**4:3 比例严格由实际布局 CSS `aspect-[4/3]` 落实**）；
...
* **移动端结算呈现（唯一 Dialog 实例）**：
  * **套餐选择与结算解耦**：点击套餐规格行仅更新当前选中项、参数与价格预览，方便比较；**只有点击底部“立即兑换”主按钮时，才拉起唯一的 `PurchaseModal`**；
  ...
  * **软键盘适配验收标准**：
    1. 聚焦输入项及其报错提示清晰可见；
    2. 表单容器可顺畅上下滚动；
    3. 提交按钮可触达（在表单底部或随滚动露出，不假定小屏下与输入框绝对同时可见）；
    4. 收起键盘后布局完整复原无留白断层；
    5. 在真实移动端浏览器完成端到端交互核验。
...
| **`provisionPending`** | `boolean` | **自动开通中**：虚线卡片提示“自动开通中，请稍候…”，并提供客服咨询入口；不作“失败必转人工交付”等无业务依据的单方承诺。 |
